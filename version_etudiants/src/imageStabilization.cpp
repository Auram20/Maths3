...
int main(int argc, char **argv)
{
  ...
  while(loop){
    ...
    //9 clamp translation between value max and min 
    double translationMax = 100;
    M(1,2) = std::min (translationMax,std::max (M(1,2),-translationMax));
    M(0,2) = std::min (translationMax,std::max (M(0,2),-translationMax));
    ...
    //9 clamp angle
    double angleMax = 15*M_PI/180;
    double angle = asin(M(0,1));
    angle = std::min(angleMax, std::max(-angleMax, angle));
    M(0,0) = cos(angle);    
    M(0,1) = sin(angle);    
    M(1,0) = -sin(angle);    
    M(1,1) = cos(angle);    
    ...
    //H = HcenteringInv * zoom * Hcentering;
